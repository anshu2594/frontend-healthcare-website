...
const Navbar = () => {
  const navItems = [
    { label: "Overview", icon: House, path: "/home" },
    { label: "About Us", icon: Info, path: "/why-us" },
    { label: "Doctors", icon: Stethoscope, path: "/doctors" },
    { label: "Departments", icon: HeartPulse, path: "/departments" },
      ...
      <nav className="flex items-center gap-1 flex-1 justify-center">
        {navItems.map(({ label, icon: Icon, path }) => (
          <NavLink
            ...
            to={path}
            className={({ isActive }) =>
              `flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-full transition-all duration-200 active:scale-95 ${
                isActive
